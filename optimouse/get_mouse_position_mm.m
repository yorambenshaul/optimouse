function Result = get_mouse_position_mm(imageIn,head_method,trim_cycles,GreyThresh_fact)
% based on get_mouse_position - the core function for detecting where the
% nose (and body center of mass) are located
% YBS 9/16
...
if prod(BB(3:4)) == numel(imageIn)
    Result.ErrorMsg = 'The detected object is the entire image';
    return
end
...
% get the intensity values of the object
Result.MouseMean = mean(tempimage(mouselinearindex));
Result.MouseRange = range(tempimage(mouselinearindex));
Result.MouseVar = var(tempimage(mouselinearindex));

% now abuse it to get the background value
...
Result.mouseCOM = mouseCOM;
Result.nosePOS  = nosePOS;
